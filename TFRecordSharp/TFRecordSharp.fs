...
module Encoding = 
    ...
    let EncodeDouble (x: double) =
        if BitConverter.IsLittleEndian then
            BitConverter.GetBytes(x)
        else    
            BitConverter.GetBytes(x) |> Array.rev

    let EncodeDoubleArray (v: double[]) =
        let buf = Array.zeroCreate<byte> (v.Length * 8)
        for i in 0 .. v.Length - 1 do
            let ba = v.[i] |> EncodeDouble
            Array.blit ba 0 buf (i*8) 8
        buf
    ...
    let DecodeDouble (buf: byte[]) =
        if BitConverter.IsLittleEndian then
            BitConverter.ToDouble(buf,0)
        else 
            let buf2 = buf |> Array.rev
            BitConverter.ToDouble(buf2,0)

    let DecodeDoubleArray (buf: byte[]) =
        let da = Array.zeroCreate<double> (buf.Length / 8)
        for i in 0 .. da.Length - 1 do
            if BitConverter.IsLittleEndian then
                da.[i] <- BitConverter.ToDouble(buf,i*8)
            else 
                let buf2 = buf.[ i*8 .. i*8 + 8 - 1] |> Array.rev
                da.[i] <- BitConverter.ToDouble(buf2,0)
        da
